  ...
  switch (method) {
    ...
    case 'mastodon':
      ...
      getOAuthApp(req, res).then(async (app) => {
        ...
        formData.append('redirect_uri', `${process.env.ENVIRONMENT === 'production' ? 'https' : req.protocol}://${req.get('host')}/callback?${queryString.stringify(internalRedirectURIparams)}`);
